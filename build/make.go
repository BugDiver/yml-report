...
const (
	...
	gauge             = "gauge"
	ymlReport         = "yml-report"
	deploy            = "deploy"
	pluginJsonFile    = "plugin.json"
)

var deployDir = filepath.Join(deploy, ymlReport)
...
func createDistro() {
	packageName := fmt.Sprintf("%s-%s-%s.%s", ymlReport, getPluginVersion(), getGOOS(), getArch())
...
func compileGoPackage(packageName string) {
	runProcess("go", "build", "-o", getGaugeExecutablePath(ymlReport))
}
...
func copyPluginFiles(destDir string) {
	...
	if getGOOS() == "windows" {
		files[filepath.Join(getBinDir(), ymlReport+".exe")] = bin
	} else {
		files[filepath.Join(getBinDir(), ymlReport)] = bin
	}
...
func installPlugin(installPrefix string) {
	...
	pluginInstallPath := filepath.Join(installPrefix, ymlReport, getPluginVersion())
