...
func addDefaultPropertiesToProject() {
	...
	if !common.FileExists(defaultPropertiesFile) {
		fmt.Printf("Failed to setup yml report plugin in project. Default properties file does not exist at %s. \n", defaultPropertiesFile)
		return
	}
	if err := common.AppendProperties(defaultPropertiesFile, reportsDirProperty, overwriteReportProperty); err != nil {
		fmt.Printf("Failed to setup yml report plugin in project: %s \n", err)
		return
	}
	fmt.Println("Succesfully added configurations for yml-report to env/default/default.properties")
}
...
func createReport(suiteResult *gauge_messages.SuiteExecutionResult) {
	jsonContents := generateYMLFileContents(suiteResult)
	reportDir, err := createYMLReport(createReportsDirectory(), jsonContents, getNameGen())
	if err != nil {
...
func generateYMLFileContents(protoSuiteExeResult *gauge_messages.SuiteExecutionResult) []byte {
...
func marshal(item interface{}) []byte {
	marshalledResult, err := yaml.Marshal(item) //json.MarshalIndent(item, "", "\t")
	if err != nil {
...
func createYMLReport(reportsDir string, jsonContents []byte, nameGen nameGenerator) (string, error) {
	var currentReportDir string
	if nameGen != nil {
		currentReportDir = filepath.Join(reportsDir, ymlReport, nameGen.randomName())
	} else {
		currentReportDir = filepath.Join(reportsDir, ymlReport)
	}
	createDirectory(currentReportDir)
	return currentReportDir, writeResultYMLFile(currentReportDir, jsonContents)
}

func writeResultYMLFile(reportDir string, jsonContents []byte) error {
